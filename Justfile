...
draw: _check_yq_version
    ...
    for keymap in "{{ config }}"/*.keymap; do
        ...
        # Map keymap filename to keyboard layout name
        case "$name" in
            sofle) keyboard="eyelash_sofle" ;;
            *)     keyboard="$name" ;;
        esac
        echo "Drawing $name ($keyboard)..."
        keymap -c "{{ draw }}/config.yaml" parse -z "$keymap" >"{{ draw }}/$name.yaml"
        keymap -c "{{ draw }}/config.yaml" draw "{{ draw }}/$name.yaml" -k "$keyboard" >"{{ draw }}/$name.svg"
    done
